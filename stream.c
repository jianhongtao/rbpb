...
extern int stropen(stream_t *stream, int type, int mode, const char *path)
{
    ...
    switch (type) {
        ...
        case STR_NCONVERTION  : stream->port=opentcpcli(path,     stream->msg); break;
...
extern int strread(stream_t *stream, unsigned char *buff, int n)
{
    ...
    switch (stream->type) {
        ...
        case STR_NCONVERTION  : nr=readtcpcli((tcpcli_t *)stream->port,buff,n,msg); break;
        case STR_NTRIPSVR:
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n)
{
    ...
    switch (stream->type) {
        ...
        case STR_NCONVERTION  : ns=writetcpcli((tcpcli_t *)stream->port,buff,n,msg); break;
        case STR_NTRIPSVR:
...
#endif
            if (!strncmp(msg+1,"HEX",3)) { /* general hex message */
